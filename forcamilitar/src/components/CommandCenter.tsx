                      ...
                      <div className="space-y-3 font-mono text-xs max-h-[300px] overflow-y-auto">
                        {pendingEnlistments.length === 0 ? (
                          <div className="text-center py-8 text-gray-500 italic">
                            Nenhum pedido de alistamento pendente.
                          </div>
                        ) : (
                          pendingEnlistments.map((req) => (
                            <div key={req.id} className="p-3 bg-fmb-slate/40 border border-fmb-army/25 rounded flex items-center justify-between gap-2">
                              <div>
                                <p className="font-bold text-white text-xs">@{req.habboNick}</p>
                                <p className="text-[9px] text-gray-400 mt-0.5">Solicitado em: {new Date(req.createdAt).toLocaleDateString("pt-BR")} às {new Date(req.createdAt).toLocaleTimeString("pt-BR")}</p>
                              </div>
                              <div className="flex items-center space-x-1.5 shrink-0">
                                <button
                                  onClick={() => handleApproveEnlistment(req.id, req.habboNick)}
                                  disabled={enlisting}
                                  className="bg-green-700 hover:bg-green-600 disabled:opacity-50 text-white font-bold px-2.5 py-1 rounded text-[10px] uppercase transition-colors"
                                  title="Aprovar com Soldado"
                                >
                                  Aprovar
                                </button>
                                <button
                                  ...
                                  disabled={enlisting}
                                  className="bg-red-900 hover:bg-red-800 disabled:opacity-50 text-white font-bold px-2.5 py-1 rounded text-[10px] uppercase transition-colors"
