...
function saveDB() {
  try {
    ...
    if (process.env.SUPABASE_URL && process.env.SUPABASE_KEY && !hasSyncedFromSupabase) {
...
export function ensureRankConfigsAndDocumentsExist() {
  ...
  if (!db.trainingCategories || db.trainingCategories.length === 0) {
...
export const dbOperations = {
  ...
  authenticateUser: (nick: string, pass: string): User | null => {
    ...
    if (hash && bcrypt.compareSync(pass, hash)) {
      isAuthed = true;
    } else if (user.habboNick.toLowerCase() === "comandante_fmb" && pass === "FMB123") {
      // Restore password tática in local DB
      const newHash = hashPassword("FMB123");
      ...
      dbOperations.addLog(user.id, user.habboNick, "SISTEMA", "A senha máster do Comandante_FMB foi reparada e sincronizada.");
      saveDB();
      isAuthed = true;
    }
  ...
  createRecruitLesson: async (
    ...
    const students = studentNick.split(",").map(s => s.trim()).filter(s => s.length > 0);
    ...
    for (const student of students) {
      ...
      if (status === "Aprovado") {
        ...
        const pendingRequest = db.enlistmentRequests.find(r => r.habboNick.toLowerCase() === student.toLowerCase());
        if (pendingRequest) {
          try {
            await dbOperations.approveEnlistmentRequest(instructorId, pendingRequest.id);
          } catch (err: any) {
  ...
  approveEnlistmentRequest: async (adminId: string, requestId: string) => {
    const admin = dbOperations.getUserById(adminId);
  ...
  addTrainingCategory: (category: string) => {
    if (!db.trainingCategories) {
      db.trainingCategories = ["Ata Básico", "Tiro Tático", "Patrulhamento", "Doutrina Básica", "Curso de Oficiais"];
    }
  ...
  deleteTrainingCategory: (category: string) => {
    ...
    db.trainingCategories = db.trainingCategories.filter(c => c.toLowerCase() !== category.trim().toLowerCase());
    saveDB();
    return db.trainingCategories;
  }
};
