...
app.put("/api/users/:id/rank", authenticateToken, (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (!isSupremo) {
      // Must be officer or have canPromote permission
      ...
      if (!hasPromotePermission && !allowedOfficers.includes(promoter.role)) {
...
app.get("/api/training-categories", authenticateToken, (req: AuthRequest, res: Response) => {
  res.json(dbOperations.getTrainingCategories());
});
...
app.post("/api/training-categories", authenticateToken, (req: AuthRequest, res: Response) => {
  ...
  const hasAccess = req.userRank === MilitaryRank.ADMSUPREMO || req.userRank === MilitaryRank.COMANDANTE_GERAL || dbOperations.hasPermission(req.userRank!, "canAdminSystem");
  if (!hasAccess) {
  ...
  try {
    const list = dbOperations.addTrainingCategory(category);
